...
static shapePtr cube(void) {
    shapePtr    cube = make_unique<Cube>();

    cube->getMaterial().colour = color(0.3, 0.3, 0.3);
    // cube->getMaterial().pattern = make_shared<Stripe>(color(0.5, 0, 0.7), ColorUtils::white());
    // cube->getMaterial().pattern = make_shared<CheckerMap>(color(0.5, 0, 0.7), ColorUtils::white());
    // cube->getMaterial().reflective = 0.9;
    cube->getMaterial().transparency = 0.9;
    cube->getMaterial().refractiveIndex = 1.5;
    cube->getMaterial().optShadow = false;
    // cube->getMaterial().pattern->setTransform(scaling(0.2, 0.2, 0.2));
    cube->getTransform().rotateX(-M_PI / 2);
    return (cube);
}

static shapePtr cube2(void) {
    shapePtr    cube = make_unique<Cube>();
    ...
    cube->getMaterial().transparency = 0.9;
    // cube->getMaterial().pattern->setTransform(translation(4, 0, 4));
    cube->getTransform().translate(4, 0, 4);
    return (cube);
}
